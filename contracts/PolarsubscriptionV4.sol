        ...
        returns (bool upkeepNeeded, bytes memory performData)
    {
        ...
        if((block.timestamp - lastTimestamp) > 1 days){     //how often should the automation run?
            ...
            for(uint i; i < _tokenIdCounter.current(); i++){
                if(idToTime[i] + 35 days < block.timestamp){ //Adjust to timeframe you would like
                    upkeepNeeded = true;
                    counter++;
                }
            }
            ...
            for(uint i; i < _tokenIdCounter.current(); i++){
                if(idToTime[i] + 35 days < block.timestamp && isSubscribed[i] == true){ //Adjust to timeframe you would like
...
//THESE FUNCTIONS INCLUDE JUST THE LOGIC OF THE CHAINLINK AUTOMATION FOR TESTING. THEY WILL BE REMOVED BEFORE DEPLOYING ON MAINNET
